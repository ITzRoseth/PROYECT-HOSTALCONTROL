...
public class EstadiaRepository {
    ...
    public void actualizarEstadoYSalida(String idEstadia, EstadoEstadia nuevoEstado,
                                    String fechaSalida, double totalPagado) {

    collection.updateOne(
            eq("_id", new ObjectId(idEstadia)),
            Updates.combine(
                    Updates.set("estado", nuevoEstado.name()),
                    Updates.set("fechaSalida", fechaSalida),
                    Updates.set("totalPagado", totalPagado)
            )
    );
}
}